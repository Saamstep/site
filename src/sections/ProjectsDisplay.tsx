...
function ProjectDisplay() {
  ...
  function filterMessage() {
    if(filterArr.length > 0) {
      return (<p className="my-2 text-left" style={{fontSize: '15px', color: '#fcba03'}}>Filtering by Tag{filterArr.length > 1 ? "s" : null}: <strong>{filterArr?.join(', ')}</strong></p>);
    } else {
    return null;
  }
}
  ...
  return (
    <Container id="projects">
      ...
      <Row className="justify-content-md-left">
        <Col md="auto"><ProjectFilters/></Col>
